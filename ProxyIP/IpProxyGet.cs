...
namespace ProxyIP
{
    ...
    public static class IpProxyGet
    {
        ...
        #region 生产IP 代理 对象
        public static HashSet<string>  Grab_ProxyIp()
        {
        ...
        public  static bool RetryFunc(Func<bool> func, int retryCount)
        {
            bool result;
            do
            {
        ...
        {
            if (string.IsNullOrEmpty(ConfigurationManager.AppSettings["IPUrl"]))
            {
                throw new ArgumentNullException("ParseProxy函数参数空异常");
            }

            //总页数
            int total = GetTotalPage(ConfigurationManager.AppSettings.Get("IPUrl"), ProxyIp);
        ...
        private static void DoWork(object param)
        {
            ...
            for (int i = start; i <= end; i++)
            {
                ...
                url = string.Format("{0}/{1}", ConfigurationManager.AppSettings.Get("IPUrl"), i);
                var doc = new HtmlAgilityPack.HtmlDocument();
                doc.LoadHtml(GetHTML(url, ProxyIp));
                //获取所有数据节点tr
                //var trs = doc.DocumentNode.SelectNodes(@"//table[@id='ip_list']/tr");
                var trs = doc.DocumentNode.SelectNodes(@"//table[@class='table']/tr");
                ...
                if (trs != null && trs.Count > 1)
                {
                    ...
                    for (int j = 1; j < trs.Count; j++)
                    {
                        ...
                        if (nodes != null && nodes.Count > 9)
                        {
                            ...
                            if (Convert.ToBoolean(ConfigurationManager.AppSettings.Get("IsPingIp")) && !Ping(ip))
                            {
                                continue;
                            }
                            //有效的IP才添加
        ...
        private static int GetTotalPage(string IPURL, string ProxyIp)
        {
            ...
            //var res = doc.DocumentNode.SelectNodes(@"//div[@id='listnav']/a");
        ...
        public static string GetCorrectIP()
        {
            ...
            while (string.IsNullOrEmpty(ProxyIp))
            {
                ...
                foreach (DataRow dr in dt.Rows)
                {
                    ...
                    if (Ping(dr["IP"].ToString()) && GetTotalPage(ConfigurationManager.AppSettings.Get("IPUrl"), tempProxyIp) > 1)
                    {
                        ProxyIp = tempProxyIp;
                        break;
                    }
            ...
            {
                ProxyIp = ConfigurationManager.AppSettings.Get("DefaultProxyIp");
            }
            return ProxyIp;
        }
